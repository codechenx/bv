...
def bv():
    ...
    supported_file_types = list(config_dict.keys())
    # argument parser
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="ignore lines with specific prefix")
    parser.add_argument(
        '-sn', default=0, type=int, help="ignore first n lines")
    parser.add_argument(
        '-rc',
        default=None,
        nargs='+',
        type=int,
        help=
        "only show columns(support for multiple arguments, separated by space) "
    )
    parser.add_argument(
        '-hc',
        default=None,
    ...
    parser.add_argument(
        '--noheader',
        action='store_true',
        help="don't use fist line as header")
    parser.add_argument(
        '--trans', action='store_true', help="view transposed data")
    parser.add_argument(
        '--compressed', action='store_true', help="file is compressed?")
    parser.add_argument('--version', action='version', version='bv 0.1.4')
    ...
    if args.rc is not None:
        if isinstance(args.rc, int):
            args.rc = [args.rc]
        rm_col = [
            i for i in range(reader.data.size[1]) if i + 1 not in args.rc
        ]
        reader.data.rm_col(rm_col)
    ...
    if args.trans:
        if args.noheader:
            ...
            reader.data.header = [
                str(i + 1) for i in range(reader.data.size[1])
            ]
        else:
